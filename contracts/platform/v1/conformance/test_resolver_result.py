...
def test_resolved_result_rejects_duplicate_manifest_dependency() -> None:
    document = copy.deepcopy(CONFORMANCE.load_source_json(FIXTURE_PATH))
    resolver_input = copy.deepcopy(CONFORMANCE.load_source_json(INPUT_FIXTURE_PATH))
    _add_module_selection(
        document,
    ...
    registry_entry = resolver_input["registry_snapshot"]["modules"][0]
    registry_entry["module"]["dependencies"].append(
        copy.deepcopy(registry_entry["module"]["dependencies"][0])
    )
    registry_entry["module"]["dependencies"][1]["optional"] = True
    registry_entry["manifest_sha256"] = hashlib.sha256(
        CONFORMANCE.jcs_canonical(registry_entry["module"])
    ).hexdigest()
    _refresh_embedded_digest(resolver_input, "registry_snapshot")
    ...
    with pytest.raises(CONFORMANCE.ConformanceError, match=r"^module_dependency"):
        CONFORMANCE.validate_resolver_result_semantics(document, resolver_input)


def test_resolved_result_rejects_manifest_self_dependency() -> None:
    document = copy.deepcopy(CONFORMANCE.load_source_json(FIXTURE_PATH))
    resolver_input = copy.deepcopy(CONFORMANCE.load_source_json(INPUT_FIXTURE_PATH))
    _add_registry_module_and_artifact_binding(document, resolver_input)
    registry_entry = resolver_input["registry_snapshot"]["modules"][0]
    registry_entry["module"]["dependencies"] = [
        {
            "module_id": "bridge.core",
            "version_constraint": "1.0.0",
            "optional": False,
...
@pytest.mark.parametrize("capability_kind", ["provides", "requires"])
def test_resolved_result_rejects_duplicate_manifest_capability(
    capability_kind: str,
) -> None:
    document = copy.deepcopy(CONFORMANCE.load_source_json(FIXTURE_PATH))
    resolver_input = copy.deepcopy(CONFORMANCE.load_source_json(INPUT_FIXTURE_PATH))
    ...
    registry_entry["module"]["capabilities"][capability_kind] = [
        capability,
    ...
    with pytest.raises(CONFORMANCE.ConformanceError, match=r"^module_capability"):
        CONFORMANCE.validate_resolver_result_semantics(document, resolver_input)
...
def test_resolved_result_rejects_duplicate_manifest_artifact_id() -> None:
    document = copy.deepcopy(CONFORMANCE.load_source_json(FIXTURE_PATH))
    resolver_input = copy.deepcopy(CONFORMANCE.load_source_json(INPUT_FIXTURE_PATH))
    _add_registry_module_and_artifact_binding(document, resolver_input)
    registry_entry = resolver_input["registry_snapshot"]["modules"][0]
    registry_entry["module"]["artifacts"].append(
        copy.deepcopy(registry_entry["module"]["artifacts"][0])
    )
    registry_entry["manifest_sha256"] = hashlib.sha256(
        CONFORMANCE.jcs_canonical(registry_entry["module"])
    ).hexdigest()
...
def test_resolved_result_rejects_manifest_artifact_path_collision() -> None:
    document = copy.deepcopy(CONFORMANCE.load_source_json(FIXTURE_PATH))
    resolver_input = copy.deepcopy(CONFORMANCE.load_source_json(INPUT_FIXTURE_PATH))
    _add_registry_module_and_artifact_binding(document, resolver_input)
    registry_entry = resolver_input["registry_snapshot"]["modules"][0]
    artifact = copy.deepcopy(registry_entry["module"]["artifacts"][0])
    artifact["artifact_id"] = "bridge.duplicate"
    registry_entry["module"]["artifacts"].append(artifact)
    registry_entry["manifest_sha256"] = hashlib.sha256(
        CONFORMANCE.jcs_canonical(registry_entry["module"])
    ).hexdigest()
    _refresh_embedded_digest(resolver_input, "registry_snapshot")
    ...
    with pytest.raises(CONFORMANCE.ConformanceError, match=r"^artifact_path_collision"):
        CONFORMANCE.validate_resolver_result_semantics(document, resolver_input)


def test_resolved_result_rejects_duplicate_configuration_requirement_without_runtime() -> None:
    document = copy.deepcopy(CONFORMANCE.load_source_json(FIXTURE_PATH))
    resolver_input = copy.deepcopy(CONFORMANCE.load_source_json(INPUT_FIXTURE_PATH))
    _add_registry_module_and_artifact_binding(document, resolver_input)
    registry_entry = resolver_input["registry_snapshot"]["modules"][0]
    ...
    registry_entry["module"]["configuration_requirements"] = [
        requirement,
